...
1. Identify one sanitized image and let the user confirm product name, condition, and optional
   description.
2. `POST /api/valuations` validates optional idempotency and Redis usage controls, then
   creates `Valuation(status=QUEUED)`.
...
- `Valuation`: idempotency, confirmed input, coarse status, price/range/confidence,
...
3. Implement opaque-ID reads, supplied `idempotencyKey` replay, and conflict detection by comparing
   stored request fields. Generate the stored key when the client omits the header.
...
3. Add platform creation/polling/evidence gateway; it may retain one idempotency key per
   submission when replay protection is needed.
